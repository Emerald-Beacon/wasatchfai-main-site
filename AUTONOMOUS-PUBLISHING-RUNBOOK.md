...
## Integration points — 7 edits per post
...
6. **`_redirects`** — regenerated by `python3 build-extensionless.py`, which adds the
   ...
   `/post/<slug>`, and carries the permanent `RETIRED` redirects. Do not hand-edit.
7. **Images** — run `python3 build-images.py`. It caps JPEGs at 1600px / ~180 KB by
   stepping quality down until they fit, and writes a `.webp` twin next to each.
   Hero backgrounds use `image-set()` with a JPEG fallback, so the WebP is served
   where supported without breaking older clients.
...
Fifteen core pages have Spanish twins under `/es/` (home, both clinics, locations,
appointments, and the ten highest-intent service pages). Every page carries a
language switch in the top bar. If you add a page to the Spanish set, add it to
`ES_SET` so the switch points at the twin rather than the Spanish home, and give
both pages reciprocal `hreflang` — a one-sided annotation is ignored by Google.

**The blog is English-only.** The routine does not translate posts. If that
changes, the Spanish surface doubles on every publish.
